...
describe('PromptModalComponent', () => {
	...
	it('should display title and message', () => {
		...
		const titleEl = fixture.debugElement.query(
			By.css('.settings-header h2'),
		).nativeElement;
		const messageEl = fixture.debugElement.query(
			By.css('.settings-header p'),
		).nativeElement;
	...
	it('should call close with value on confirm', () => {
		...
		const confirmBtn = fixture.debugElement.queryAll(
			By.css('app-button'),
		)[1]; // Second button is confirm
	...
	it('should focus textarea after view init', fakeAsync(() => {
		...
		if (textAreaComponent?.textareaRef) {
			const focusSpy = jasmine.createSpy('focus');
			const mockNativeElement = { focus: focusSpy };

			// Mock the signal textareaRef()
			(textAreaComponent as any).textareaRef = signal({
