...
export class BookService {
	...
	getHomeBooksData(bookIds: string[]): Observable<{
		...
		const query = `
			query GetHomeBooks($filterUpdated: BookFilterInput, $filterAdded: BookFilterInput, $hasContinueReading: Boolean!, $filterContinue: BookFilterInput) {
				...
				continueReading: books(filter: $filterContinue) @include(if: $hasContinueReading) {
					data {
						id title description cover tags { id name } scrapingStatus covers { url isMain metadata { blurHash dominantColor } }
					}
				}
			...
			map((books) => {
				...
				filteredBooks = filteredBooks.filter((book) => {
					...
					if (
						...
						book.sensitiveContent.length === 0
					)
						return true;
	...
	getAllChapters(bookId: string, limit = 200): Observable<Chapterlist[]> {
		return from(this.loadAllChapters(bookId, limit));
	}
	...
	private async loadAllChapters(
		...
		while (hasNextPage) {
			const page = await firstValueFrom(
				this.getChapters(bookId, { cursor, limit }),
			);
	...
	selectCover(bookId: string, coverId: string) {
		return this.http.patch<Book>(
			`books/${bookId}/covers/${coverId}/selected`,
			{},
		);
	}

	updateCover(bookId: string, coverId: string, data: { title: string }) {
		return this.http.patch<Cover>(
			`books/${bookId}/covers/${coverId}`,
			data,
		);
	}
	...
	replaceCoverImage(
		...
		return this.http.patch<Cover>(
			`books/${bookId}/covers/${coverId}/image`,
			formData,
		);
	}
	...
	downloadBook(
		...
		return new Observable((observer) => {
			...
			const baseUrl =
				environment.apiURL || `${window.location.origin}/api`;
