...
export class ReadingProgressService {
		...
		return new Promise((resolve, reject) => {
			...
			request.onupgradeneeded = (event) => {
				...
				if (!db.objectStoreNames.contains(this.syncStoreName)) {
					db.createObjectStore(this.syncStoreName, {
						keyPath: 'chapterId',
					});
				}
			};
	...
	private async executeSaveProgress(
		...
		try {
			...
			return new Promise((resolve, reject) => {
				...
				request.onsuccess = () => {
					console.log(
						`✅ Progresso salvo para usuário ${targetUserId}, capítulo ${chapterId}, página ${pageIndex}`,
					);
					resolve();
				};
	...
	async migrateGuestProgressToUser(userId: string): Promise<void> {
		...
		console.log(
			`🔄 Migrando ${guestProgress.length} progressos do guest para usuário ${userId}...`,
		);

		for (const progress of guestProgress) {
			// Verifica se o usuário já tem progresso para este capítulo
			const userProgress = await this.getProgress(
				progress.chapterId,
				userId,
			);
			...
			if (!userProgress || progress.pageIndex > userProgress.pageIndex) {
				...
				console.log(
					`✅ Migrado: ${progress.chapterId} (página ${progress.pageIndex})`,
				);
			}
	 ...
	 */
	async clearSyncedProgress(
		chapterIds: string[],
		userId?: string,
	): Promise<void> {
		if (!this.isBrowser || !this.dbPromise || chapterIds.length === 0)
			return;
