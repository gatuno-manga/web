...
export class UnifiedReadingProgressService implements OnDestroy {
    ...
    private pendingSyncData: { chapterId: string; bookId: string; pageIndex: number; totalPages?: number; completed?: boolean } | null = null;
    ...
    private debounceSyncToApi(
        ...
        this.pendingSyncData = { chapterId, bookId, pageIndex, totalPages, completed };
     ...
     */
    async getLastProgressForBook(bookId: string): Promise<ReadingProgress | undefined> {
        if (!this.isBrowser) return undefined;
    ...
    async syncAll(): Promise<void> {
        if (!this.isBrowser || !this.userTokenService.hasValidAccessToken) return;
    ...
    async syncLocalHistoryToServer(): Promise<void> {
        if (!this.isBrowser || !this.userTokenService.hasValidAccessToken) {
            console.log('⚠️ Não é possível sincronizar: usuário não autenticado');
            return;
        }
    ...
    async onUserLogin(): Promise<void> {
        ...
        const userId = this.extractUserIdFromToken();
        if (!userId) {
            console.error('❌ Não foi possível extrair o ID do usuário do token');
            return;
        }
