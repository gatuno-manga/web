...
@Component({
	...
	standalone: true,
	imports: [ButtonComponent, ListCheckboxComponent, IconsComponent, FormsModule, RouterLink, DragDropModule, TooltipDirective],
...
export class SensitiveContentComponent {
	...
	loadSensitiveContents() {
		...
		this.sensitiveContentService.getSensitiveContent().subscribe((contents) => {
			this.ngZone.run(() => {
				// Sort items descending by weight so highest weight is at the top
				const sorted = contents.sort((a, b) => (b.weight || 0) - (a.weight || 0));
	...
	onDrop(event: any) {
		...
		moveItemInArray(currentContents, event.previousIndex, event.currentIndex);
		...
		if (batchItems.length > 0) {
			this.sensitiveContentService.updateSensitiveContentBatch(batchItems).subscribe();
		}
	...
	createContent() {
		...
		this.sensitiveContentService.createSensitiveContent(this.newContentName, nextWeight).subscribe(() => {
			this.ngZone.run(() => {
				this.loadSensitiveContents();
				this.isCreating = false;
				this.newContentName = '';
	...
	updateContent(id: string) {
		...
		this.sensitiveContentService.updateSensitiveContent(id, this.editContentName, finalWeight).subscribe(() => {
			this.ngZone.run(() => {
				this.loadSensitiveContents();
				this.editingId = null;
				this.cdr.detectChanges();
	...
	deleteContent(id: string) {
		if (confirm('Tem certeza que deseja excluir este conteúdo sensível?')) {
			this.sensitiveContentService.deleteSensitiveContent(id).subscribe(() => {
				this.ngZone.run(() => {
					this.loadSensitiveContents();
					this.cdr.detectChanges();
				});
	...
	mergeContents() {
		...
		const selectedTags = this.mergingTags.filter((t) => t.checked).map((t) => t.id);
