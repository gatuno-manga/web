...
describe('InfoBookComponent', () => {
	...
	beforeEach(async () => {
		...
		mockBookService = {
			getChapters: jasmine.createSpy('getChapters').and.returnValue(
				of({
					data: [{ id: 'c1', title: 'Chapter 1', index: 1 }],
					nextCursor: null,
					hasNextPage: false,
				}),
			),
			getCovers: jasmine
				.createSpy('getCovers')
				.and.returnValue(of([{ id: 'cv1', url: 'http://img' }])),
			getInfo: jasmine.createSpy('getInfo').and.returnValue(
				of({
					alternativeTitle: [],
			...
			selectCover: jasmine
				.createSpy('selectCover')
				.and.returnValue(EMPTY),
			watchBook: jasmine
				.createSpy('watchBook')
				.and.returnValue(watchSubject.asObservable()),
		};
	...
	it('selectTab triggers module load function when not loaded', async () => {
		...
		component.modulesLoad[index].function = jasmine
			.createSpy('fn')
	...
	it('selectCover should show modal and call selectCover', (done) => {
		...
		setTimeout(() => {
			expect(mockBookService.selectCover).toHaveBeenCalledWith(
				'book-1',
				'cv1',
			);
			done();
