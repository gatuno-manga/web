...
export class ItemBookComponent {
  ...
  onContextMenu(event: MouseEvent) {
    ...
    this.downloadService.isBookDownloaded(this.book.id).then(isDownloaded => {
      if (isDownloaded) {
        items.push(
          { type: 'separator' },
          {
  ...
  private async confirmDownloadBook() {
    try {
      ...
      if (chapters.length === 0) {
        this.modalService.show('Erro', 'Este livro não possui capítulos.', [{ label: 'Ok', type: 'primary' }], 'error');
        return;
      }
      ...
      for (const chapterInfo of chapters) {
          try {
              ...
              await this.downloadService.downloadChapter(fullBook as any, fullChapter);
              downloadedCount++;
      ...
      this.modalService.show('Erro', 'Erro ao iniciar download.', [{ label: 'Ok', type: 'primary' }], 'error');
    }
  }
