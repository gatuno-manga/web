...
export class BookFilterComponent implements OnInit {
    ...
    // Lists
    availableTypes = Object.values(TypeBook).map(type => ({ value: type, label: type.toUpperCase() }));
    ...
    // Sorting
    orderBy = signal<'title' | 'createdAt' | 'updatedAt' | 'publication'>('createdAt');
    ...
    activeFilters = computed(() => {
        const filters: { id: string, label: string, type: string, value: any }[] = [];

        if (this.searchValue()) {
            filters.push({ id: 'search', label: `Busca: ${this.searchValue()}`, type: 'search', value: this.searchValue() });
        }

        this.selectedTypes().forEach(type => {
            filters.push({ id: `type-${type}`, label: type.toUpperCase(), type: 'type', value: type });
        });
        ...
        this.excludedTags().forEach(tagId => {
            const tag = this.availableTags().find(t => t.id === tagId);
            if (tag) {
                filters.push({
        ...
        this.selectedSensitiveContent().forEach(id => {
            const content = this.availableSensitiveContent().find(c => c.id === id);
            if (content) {
                filters.push({ id: `sensitive-${id}`, label: content.name, type: 'sensitive', value: id });
            }
        });

        if (this.publicationYear()) {
            const operatorLabel = this.publicationOperators.find(o => o.value === this.publicationOperator())?.label;
            filters.push({
                id: 'publication',
    ...
    private fetchData() {
        this.sensitiveContentService.getSensitiveContent().subscribe(content => {
            const allowedNames = this.sensitiveContentService.getContentAllow();
            // Filter available options based on allowed content
            const filteredContent = content.filter(c => allowedNames.includes(c.name));
            this.availableSensitiveContent.set(filteredContent);
    ...
    private getTagsObservable(): Observable<TagResponse[]> {
        ...
        if (selectedIds.length > 0) {
            ...
            sensitiveContent = this.availableSensitiveContent().map(content => content.name);
        }
    ...
    removeFilter(filter: any) {
        if (filter.type === 'search') {
            ...
            this.selectedTypes.update(types => types.filter(t => t !== filter.value));
        } else if (filter.type === 'tag') {
            this.selectedTags.update(tags => tags.filter(t => t !== filter.value));
        } else if (filter.type === 'excludeTag') {
            this.excludedTags.update(tags => tags.filter(t => t !== filter.value));
        } else if (filter.type === 'sensitive') {
            this.selectedSensitiveContent.update(content => content.filter(c => c !== filter.value));
            this.fetchTags();
        } else if (filter.type === 'publication') {
            this.publicationYear.set(undefined);
        } else if (filter.type === 'author') {
            this.selectedAuthors.update(authors => authors.filter(a => a !== filter.value));
        }
        this.emitFilters();
    }
    ...
    onPublicationOperatorChange(value: string) {
        this.publicationOperator.set(value as any);
    }
    ...
    onSortChange(value: string) {
        this.orderBy.set(value as 'title' | 'createdAt' | 'updatedAt' | 'publication');
    }
    ...
    private emitFilters(randomBook: boolean = false) {
