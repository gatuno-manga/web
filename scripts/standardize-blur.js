...
const blurRegex = /([ \t]*)backdrop-filter:\s*blur\(([^)]+)\)(?:\s*saturate\(([^)]+)\))?;(?:[ \t]*\n[ \t]*-webkit-backdrop-filter:[^;]+;)?/g;
...
allFiles.forEach(filePath => {
	...
	content = content.replace(blurRegex, (match, indent, blurVal, satVal) => {
		modified = true;
	...
	if (modified) {
		...
		if (!content.includes("@use") && !content.includes("@import 'mixins'") && !content.includes('@import "mixins"')) {
			content = `@import 'mixins';\n${content}`;
		}
		
		fs.writeFileSync(filePath, content, 'utf8');
		console.log(`✅ Atualizado: ${path.relative(path.join(__dirname, '..'), filePath)}`);
		updatedFiles++;
	}
});
