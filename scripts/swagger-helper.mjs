...
async function main() {
    ...
    switch (command) {
        ...
        case '--model':
            ...
            break;
        default:
            console.log('Usage: node scripts/swagger-helper.mjs [--summary | --path <path> | --model <model> | --fetch <url>]');
    }
}
...
function printSummary(swagger) {
    ...
    for (const [path, methods] of Object.entries(swagger.paths)) {
        for (const [method, detail] of Object.entries(methods)) {
            console.log(`${method.toUpperCase().padEnd(7)} ${path.padEnd(40)} | ${detail.summary || detail.operationId || ''}`);
        }
    }
}
